...
| `updateQuantity(id, quantity)` | Оновлює кількість; якщо `quantity <= 0` — видаляє товар        | `id` — унікальний ідентифікатор, `quantity` — нова кількість             |
...
| `getSummary()`                 | Повертає повну інформацію про кошик { items, totalQuantity, totalPrice }                            | —                                                                        |
