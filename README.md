...
| ------------------------------ | ----------------------------------- | ------------------------------------------------- |
| `addItem(item)`                | Додає товар                         | `item` — об’єкт товару з полями:                |
|                                |                                     | `id`, `price`, `quantity` та додаткові          |
| `updateQuantity(id, quantity)` | Оновлює кількість товару            | `id` — унікальний ідентифікатор                 |
...
| `getSummary()`                 | Повертає об’єкт кошика              | `{ items, totalQuantity, totalPrice }`           |
| `onChange(callback)`           | Підписка на зміни кошика            | `callback(items)` — функція, де `items` — стан  |
