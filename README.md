...
| ------------------------------ | ----------------------------------- | ------------------------------------------------------------------------ |
| `addItem(item)`                | Додає товар                         | `item` — об’єкт товару з полями:<br>`id`, `price`, `quantity` та додаткові |
| `updateQuantity(id, quantity)` | Оновлює кількість товару            | `id` — унікальний ідентифікатор<br>`quantity` — нова кількість             |
| `removeItem(id)`               | Видаляє товар                       | `id` — унікальний ідентифікатор                                          |
| `clear()`                      | Очищає кошик                        | —                                                                        |
| `getItems()`                   | Повертає масив товарів              | —                                                                        |
| `getTotalQuantity()`           | Повертає загальну кількість         | —                                                                        |
| `getTotalPrice()`              | Повертає загальну суму              | —                                                                        |
| `getSummary()`                 | Повертає об’єкт кошика<br>`{ items, totalQuantity, totalPrice }` | —                                                                        |
| `onChange(callback)`           | Підписка на зміни кошика            | `callback(items)` — функція, де `items` — поточний стан кошика           |
| `offChange(handler)`           | Відписка від змін                   | `handler` — обробник, повернений `onChange`                              |
